    ...
    <div className="flex flex-col items-center h-full text-black dark:text-white text-center fadeIn">
      ...
      <div className="grid gap-8 grid-cols-1 md:grid-cols-2 auto-rows-auto mt-12">
        ...
        <LandingCard
          ...
          stays readable and scalable. Also, there are no external dependencies."
        />
        <LandingCard
          title="Configure"
...
function Button({ onClick, title }: { onClick: () => void; title: string }) {
  return (
    <button
      onClick={onClick}
      className={`hover:bg-gray-200 transition-all font-medium text-sm border-slate-200 rounded-md px-6 py-3 self-start text-black mt-10 bg-white select-none`}
    >
      {title}
    </button>
  )
